...
MongoREST.init = function(url){
    this.url = url;
    return this;
};

MongoREST.queryData = function(query, callback) {
...
MongoREST.insertRecord = function(recordIn ,callback){
...
MongoREST.insertRecordArray = function(records, callback){
    var docs = [];
...
MongoREST.insertFile = function(){};
MongoREST.insertFileArray = function(){};

MongoREST.updateData = function(query, update, callback){
    client.connect(this.url, function (err, db){
        if(err) { callback(err); return; }
        var col = db.collection(collection);
